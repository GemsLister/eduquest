  ...
  const fetchAndAnalyze = async (quizId) => {
    try {
      ...
      const { data: attempts, error: aError } = await supabase
        .from("quiz_attempts")
        .select(
          "id, score, user_id, guest_name, student_id, student_name, student_email",
        )
        .eq("quiz_id", quizId);
      ...
      // Get unique user IDs from attempts (for instructor/auth users)
      const userIds =
        attempts?.filter((a) => a.user_id).map((a) => a.user_id) || [];

      // Fetch profile info for authenticated users
      let profilesMap = {};
      if (userIds.length > 0) {
      ...
      // Create a map of attempt_id to taker info
      // Supports: authenticated users (user_id), guests (guest_name), and students (student_name)
      const takersMap = {};
      attempts?.forEach((att) => {
        let takerName = "Anonymous";
        let takerEmail = "";

        if (att.student_name) {
          // Student attempt via public quiz link (post-merge)
          takerName = att.student_name;
          takerEmail = att.student_email || "";
        } else if (att.user_id && profilesMap[att.user_id]) {
          ...
          // Legacy guest attempt
          takerName = att.guest_name;
        }
      ...
      const results = questions.map((q) => {
        ...
        if (qResponses && qResponses.length >= 2) {
          ...
          upperCorrect = qResponses.filter(
            (r) =>
              upperGroup.some((u) => u.attemptId === r.attempt_id) &&
              r.is_correct,
          ).length;
          lowerCorrect = qResponses.filter(
            (r) =>
              lowerGroup.some((l) => l.attemptId === r.attempt_id) &&
              r.is_correct,
          ).length;
        ...
        if (q.type === "mcq" && q.options && qResponses.length > 0) {
          ...
          Object.keys(distractorAnalysis).forEach((key) => {
            distractorAnalysis[key].percentage =
              qResponses.length > 0
                ? (
                    (distractorAnalysis[key].count / qResponses.length) *
                    100
                  ).toFixed(1)
                : 0;
          });
            ...
            {!loading && analysis.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-400 text-sm mt-2">
                  Students need to take the quiz first to generate analysis
                  data.
                </p>
              </div>
            ) : (
                    ...
                    {analysis.map((item, idx) => (
                      <>
                        ...
                        >
                          ...
                          <td className="p-4 text-center">
                            {(item.distractorAnalysis || item.takersDetails) &&
                                ...
                                {item.takersDetails &&
                                  item.takersDetails.length > 0 && (
                                    <div>
                                      ...
                                      {(() => {
                                        const currentPage =
                                          takersPage[item.question_id] || 1;
                                        const itemsPerPage = 10;
                                        const totalPages = Math.ceil(
                                          item.takersDetails.length /
                                            itemsPerPage,
                                        );
                                        const startIndex =
                                        ...
                                        const paginatedTakers =
                                          item.takersDetails.slice(
                                            startIndex,
                                            endIndex,
                                          );
