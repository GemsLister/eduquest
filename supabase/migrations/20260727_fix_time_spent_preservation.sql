-- Fix both save_quiz_response and submit_quiz_attempt to preserve time_spent_seconds
-- This ensures that time tracking data is not lost when saving answers or submitting quiz

-- 1) Fix save_quiz_response to accept and accumulate time_spent_seconds
CREATE OR REPLACE FUNCTION save_quiz_response(
  p_attempt_id UUID,
  p_question_id UUID,
  p_answer TEXT,
  p_time_spent NUMERIC(12, 1) DEFAULT 0
) RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Get existing response and its time spent
  SELECT id, time_spent_seconds INTO v_existing_id, v_existing_time
    FROM quiz_responses
   WHERE attempt_id = p_attempt_id
     AND question_id = p_question_id;

  -- Calculate total time (existing + new)
  v_total_time := COALESCE(v_existing_time, 0) + COALESCE(p_time_spent, 0);
  ...
  ELSE
    ...
    VALUES (p_attempt_id, p_question_id, p_answer, v_is_correct, v_points_earned, v_total_time);
  END IF;
END;
$$;

-- 2) Fix submit_quiz_attempt to preserve time_spent_seconds
