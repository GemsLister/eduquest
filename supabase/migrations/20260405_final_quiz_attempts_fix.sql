-- Final correct fix for quiz_attempts table without breaking existing data
...
DO $$
BEGIN
    -- Check if column exists before cleaning data
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public'
        AND table_name = 'quiz_attempts' 
        ...
        -- Column exists, so clean invalid references
...
-- Step 4: Add proper foreign key constraint (only if user_id column exists)
DO $$
BEGIN
    ...
    IF EXISTS (
        ...
        -- Column exists, so add constraint
...
DO $$
BEGIN
    -- Check if column exists and no NULL values
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public'
        AND table_name = 'quiz_attempts' 
        ...
        -- Column exists and no NULL values, so make it NOT NULL
